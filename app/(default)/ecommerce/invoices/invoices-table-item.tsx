...
export default function InvoicesTableItem({ invoice, onCheckboxChange, isSelected }: InvoicesTableItemProps) {
  ...
  return (
    <tr>
      ...
      <td className="whitespace-nowrap px-2 py-3 first:pl-5 last:pr-5">
        <div className={`inline-flex rounded-full px-2.5 py-0.5 text-center font-medium ${statusColor(invoice.status)}`}>{invoice.status}</div>
      </td >
      <td className="whitespace-nowrap px-2 py-3 first:pl-5 last:pr-5">
      ...
      <td className="w-px whitespace-nowrap px-2 py-3 first:pl-5 last:pr-5">
        <div className="space-x-1">
          ...
          <button className="rounded-full text-rose-500 hover:text-rose-600">
