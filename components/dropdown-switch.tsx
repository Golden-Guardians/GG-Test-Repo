    ...
    <>
      <Menu as="div" className="relative">
        <Menu.Button className="flex grow items-center truncate">
          <Image className="mr-2 h-8 w-8 rounded-full" src={options[selected].image} width={32} height={32} alt="Group 01" />
          <div className="truncate">
            <span className="text-sm font-medium group-hover:text-slate-800 dark:text-slate-300 dark:group-hover:text-slate-200">{options[selected].channel}</span>
          </div>
          ...
          <Menu.Items as="ul" className="focus:outline-none">
            {options.map((option, optionIndex) => (
              <Menu.Item key={optionIndex} as="li">
                {({ active }) => (
                  <button
                    className={`block w-full px-3 py-1.5 text-sm font-medium ${active ? 'text-slate-800 dark:text-slate-200' : 'text-slate-600 dark:text-slate-300'}`}
                    onClick={() => { setSelected(option.id) }}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex grow items-center truncate">
                        <Image className="mr-2 h-7 w-7 rounded-full" src={option.image} width={28} height={28} alt="Channel 01" />
                        <div className="truncate">{option.channel}</div>
                      </div>
                      <svg className={`ml-1 h-3 w-3 shrink-0 fill-current text-indigo-500 ${option.id !== selected && 'invisible'}`} viewBox="0 0 12 12">
