...
export default function DropdownProfile({ align }: {
  ...
  return (
    <Popover className="relative inline-flex">
      ...
      <Transition
        className={`absolute left-0 right-auto top-full z-10 mt-1 min-w-[14rem] origin-top-right overflow-hidden rounded border border-slate-200 bg-white pt-1.5 shadow-lg dark:border-slate-700 dark:bg-slate-800 ${align === 'right' ? 'md:left-auto md:right-0' : 'md:left-0 md:right-auto'
          }`}
      ...
      >
        <Popover.Panel>
          {({ close }) => (
            <>
              <div className="px-3 pb-2 pt-1.5 text-xs font-semibold uppercase text-slate-400 dark:text-slate-500">Filters</div>
              <ul className="mb-4">
              ...
              <div className="border-t border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-700 dark:bg-slate-700/20">
                <ul className="flex items-center justify-between">
                  <li>
                    <button className="btn-xs border-slate-200 bg-white text-slate-500 hover:border-slate-300 hover:text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:border-slate-600 dark:hover:text-slate-200">Clear</button>
                  </li>
                  <li>
                    <button className="btn-xs bg-indigo-500 text-white hover:bg-indigo-600" onClick={() => close()}>Apply</button>
                  </li>
                </ul>
              </div>
            </>
